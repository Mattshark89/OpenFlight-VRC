...
namespace OpenFlightVRC.Integrations.Cyan.PlayerObjectPool
{
    ...
    public class CyanPlayerObjectPoolEditor : Editor
    {
        ...
        // Maximum world capacity is ~82
        private const int MaxPoolObjects = 82;
                ...
                if (cyanPlayerObjectPool.autoPoolSize)
                {
                    try
                    {
                        ...
                        if (cyanPlayerObjectPool.poolSize != world.Capacity + 2)
                        {
                            ...
                            Debug.Log("Automatically adjusting pool to match world size (" + cyanPlayerObjectPool.poolSize + ")");
                    ...
                    {
                        Debug.LogWarning("Failed to auto size pool! Pool size may not match world size. Setting to the maximum pool size instead. (" + MaxPoolObjects + ")");

                        cyanPlayerObjectPool.poolSize = MaxPoolObjects;
                    }
                }
            }
        }
